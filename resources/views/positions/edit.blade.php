...
<div class="content-header">
    ...
    <form id="edit-form" action="{{ route('positions.update', ['position' => $position]) }}" method="POST">
    @method('PATCH')
        @csrf
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
                    ...
                    <input type="text" name="position" class="form-control" required placeholder="Position" value="{{$position->position}}">
                </div>
            </div>
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
                    ...
                    <select name="priority" class="form-control" id="positions">
                      <option value = {{$lowest_priority}}> Next Lowest Priority: {{$lowest_priority}} </option>
                      @forelse($available_priorities as $available)
                      <option value = "{{ $available }}">Missing Priority: {{ $available }}</option>
                      @empty
                      @endforelse
                      <option value = "{{ $position->priority }}"  selected >{{ $position->priority }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="row">
              ...
              <b>Created at :</b> {{$position->created_at}}
              </div>
              <div class="col-xs-6 col-sm-6 col-md-6 text-center">
              <b>Updated at :</b> {{$position->updated_at}}
              </div>
              </div>
            </div>
